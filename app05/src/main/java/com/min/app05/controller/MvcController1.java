...
public class MvcController1 {
  ...
  public String login(HttpServletRequest request // 모든 요청 정보(요청 파라미터, 세션)를 관리하는 인터페이스
  ) {
    // 요청 파라미터 받기
    ...
    // 세션 아이디를 확인해 본다. (브라우저의 쿠키 : F12-Application-Cookies에서 JSESSIONID 쿠키로 사용자에게
    // 전달되어 저장된다.)
    // System.out.println(session.getId());

    // 세션 유효시간 설정하기.(초단위로 설정한다. 시간이 지나면 새로운 세션을 발급 받는다.)
    // 세션의 기본 유효시간은 1800초 입니다. (1800초 = 30분)
    // session.setMaxInactiveInterval(5); // 5초 후에 소멸되는 세션

    // id와 pw를 이용해서 UserVo 인스턴스를 만든다.
    ...
    if(id.equals(pw)) {
      ...
      // 세션에 저장할 때도 속성(Attribute)으로 저장한다.
